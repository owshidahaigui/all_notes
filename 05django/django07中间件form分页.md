...
- 中间件类:
    ...
    - 中间件类须实现下列五个方法中的一个或多个:
        - `def process_request(self, request):` 执行url匹配之前被调用，在每个请求上调用，返回None或HttpResponse对象 
        - `def process_view(self, request, callback, callback_args, callback_kwargs):` 调用视图之前被调用，在每个请求上调用，返回None或HttpResponse对象
        - `def process_response(self, request, response):` 所有响应返回浏览器之前被调用，在每个请求上调用，只能返回HttpResponse对象，不然浏览器无法接收响应
    ...
    - 注： 中间件中的大多数方法在返回None时表示忽略当前操作进入下一项事件（正常执行），当返回HttpResponese对象时表示此请求结果，直接返回给客户端（不再执行，URL或者视图函数之类）
...
- 练习
    - 用中间件实现强制某个IP地址只能向/test 发送5次GET请求
    - 提示:
...
4. 在模板中解析form对象
    ...
    2. 自动解析
        ...
        3. {{form.as_table}}
            ```
            将 form 中的每个属性(控件/文本)都使用文字<tr>输入框<th>标记包裹起来再显示
            注意:必须手动提供table标记
            ```
...
7. Field 内置小部件 - widget
    1. 什么是小部件
        ...
        upwd=forms.CharField(widget=forms.PasswordInput)#在forms模块里
        ```
    2. 常用的小部件类型
        | widget名称 | 对应和type类值 |
        |-|-|
        | TextInput(默认) | type='text' |
...
- 验证步骤:(增加函数,里面加逻辑判断,增加is_valid() 方法的合法条件)
    1. 先对form.XXXField() 参数值进行验证，比如:min_length,max_length, validators=[...],如果不符合form.is_valid()返回False
    
    2. 对**单个属性检验**from.clean_zzz属性名(self): 方法对相应属性进行验证,如果验证失败form.is_valid()返回False
    
       自定义方法,zzz为对应判断的属性名,如clean_username,检查用户名,
    
    3. 对**多个属性检验**form.clean(self): 对表单的整体结构进行验证，如果验证失败form.is_valid()返回False
...
- 小结:只要模板中的表单属性名与表单类的属性名相同就都可以   类名(request.POST).is_valid 来检测,不一定要用表单类生成的表单.
...
- 对象的构造方法
    ...
    - 参数
        - object_list 对象列表(可以用Book.objects.all())
        - per_page 每页数据个数
    - 返回值:
        - 分页对象
...
- 分页示例:
    ...
    <a href="{% url "book" %}?page={{ page.previous_page_number }}">上一页</a>
        {# 上面{%URL 'book'%} 表示反向解析路由,book为路由别名,在urls.py中设置#}
    {% else %}
    上一页
{% endif %}
