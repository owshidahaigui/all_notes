...
+ "文字环绕":浮动元素遮挡正常元素的位置,无法遮挡正常内容（文本之类）的显示,内容围绕在浮动元素周围显示
#### 4）常见问题 
...
```
参数可以是像素单位，也可以是%百分号。
百分号使用时，相对定位就是以父元素为标准，绝对定位要父元素是相对定位是才能参照父元素
- 另外绝对定位元素使用外边距微调位置时，方向要与偏移方向相同，比如绝对定位left:50%;margin-left:-50px;
都用left方向，50px,为子元素的水平宽度的一半
#### 4）分类 
+ relative 相对定位
```text
元素设置相对定位,可参照元素在文档中的原始位置进行偏移,**不会脱离文档流**，原始位置占位
```
+ absolute 绝对定位
```text
1. 绝对定位的元素参照离他最近的已经定位的祖先元素进行偏移,如果没有,则参照窗口进行偏移
2. 绝对定位的元素会**脱流**,在文档中不占位,可以手动设置宽高
```
使用绝对定位 :
...
```text
1. 像素值
	设置背景图片的起始坐标(图片左上角)在模块元素坐标系中的x,y位置。
	默认图片的左上角在元素0,0位置，即元素的左上角，x向上增长，y向右增长。
2. 方位值
