...
np.zeros(数组元素个数, dtype='类型')默认元素为0
...
**就地变维：直接改变原数组对象的维度，不返回新数组**resize()
...
mp.figure方法不仅可以构建一个新窗口，如果已经构建过title='xxx'的窗口，又使用figure方法构建了title=相同 的窗口的话，mp将不会创建新的窗口，而是把title='xxx'的窗口置为当前操作窗口。
...
![matplotlib_cmap](images/matplotlib_cmap.png)
...
- 作用：
    - 求行平均数，最小值，最大值，总数等
        求列平均数，最小值，最大值，总数等
- 函数：np.apply_along_axis(func, axis, array)
    - axis      等于时1表示行，0表示列
...
def func(array):
    return array.mean(),array.max()#array.min(),可以求每行或列的各种值
# 汇总arr数组每一行的均值,最大值
r=np.apply_along_axis(func,1,arr)
print(r)
#[[ 3.  5.]
 [ 8. 10.]
 [13. 15.]
 [18. 20.]]
# 汇总arr数组每一列的均值和最大值
r=np.apply_along_axis(func,0,arr)
print(r)
...
- 作用：求x轴变化，运算也在变化的操作，类似移动均线
- 函数：c = numpy.convolve(a, b, 卷积类型)
    - 卷积类型：valid,same,full
...
```python
a = [1 2 3 4 5]  原数组
b = [6 7 8]		 卷积核数组,长度设置奇数
c = numpy.convolve(a, b, 卷积类型)
#原数组1前面的0为补充数字，当2个数组有一个数还能在对应上就计算结果为完全卷积结果
#卷积核数组的中间数对应为有效数，计算的结果为同维卷积结果，长度与原数组相同
#卷积核数组的所有数都是对应原数组的有效数时，计算的结果为有效卷积结果
                40  61  82         - 有效卷积结果(valid)
            19  40  61  82  67     - 同维卷积结果(same)
        6   19  40  61  82  67  40 - 完全卷积结果(full)
#结果为原数组和卷积核数组的数，对应位置的数相乘再相加
0   0   1   2   3   4   5   0   0  #原数组，第一个数1与卷积核数组的第一个数排在一列
8   7   6	#卷积核数组反向排序
    8   7   6	#卷积数组右移
        8   7   6
            8   7   6
...
\end{array} 
\right ]
\\A \qquad\qquad C \qquad\quad B
$$

解决函数：**np.linalg.lstsq(A,B)**
...
线性拟合可以寻求与**一组散点走向趋势规律相适应**的线型表达式方程(一元一次方程)。
...
- **作用：通过==两组==统计数据计算而得的协方差可以评估这两组统计数据的相似程度。**
...
```python
cov_ab = ave(dev_a x dev_b)#离差相乘求平均数
...
mp.title('COV LINES', f公式ontsize=16)
mp.xlabel('x', fontsize=14)
mp.ylabel('y', fontsize=14)
x = np.arange(0, 10)
#a,b两条线
...
**协方差除去两组统计样本标准差的乘积**是一个[-1, 1]之间的数。该结果称为统计样本的相关系数。
...
```
根据多项式函数(P数组即可以理解为一个多项式函数)与自变量求出拟合值, 由此可得拟合曲线坐标样本数据 [X, Y']
np.polyval(P, X)->Y'

多项式函数求导，根据拟合系数求出多项式函数导函数的系数数组
np.polyder(P)->Q 

已知多项式系数P 求多项式函数的根（与x轴交点的横坐标,y=0）
xs = np.roots(P)
...
3. 把该曲线的拐点都标出来。
'''
dates, bhp_closing_prices = np.loadtxt('../../data/bhp.csv', 		                           delimiter=',',usecols=(1, 6), unpack=True, 
...
**数据的平滑处理通常包含有(卷积)降噪、拟合等操作**。降噪的功能意在去除额外的影响因素，拟合的目的意在数学模型化，可以通过更多的数学方法识别曲线特征。

案例：绘制两只股票收益率曲线。收益率 =（后一天收盘价-前一天收盘价） / 前一天收盘价

- np.diff（array） 后一个元素减前一个元素的差

1. 使用卷积完成数据降噪。

```python
dates, bhp_closing_prices 
= np.loadtxt( '../data/bhp.csv', delimiter=',', 	 										usecols=(1,6), dtype='M8[D], 												f8',converters={1:dmy2ymd}, 												unpack=True)

vale_closing_prices 
= np.loadtxt( '../data/vale.csv', delimiter=',', usecols=(6), 		  	                    dtype='f8',converters={1:dmy2ymd}, unpack=True)
#获取两支股票的收益率
#np.diff（array） 后一个元素减前一个元素的差
...
```python
# 把foo转换成矢量函数，该矢量函数收n个参数，返回m个结果 
fun = np.frompyfunc(foo, n, m)
fun(X, Y)
```
